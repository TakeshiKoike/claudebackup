# -*- coding: utf-8 -*-
"""
v3: v2をベースに、グループ化シェイプ内のテキストも再帰的に翻訳する
"""
import sys, io
...
out_path = r"C:\Users\kokek\OneDrive\デスクトップ\koike  KES2025_Nursing_Metaverse_Presentation_JP_v3.pptx"
...
translations = {
    ...
    # === Slide 9/10/11 ===
    "Example of a digital simulated patient using generative AI": "生成AIを用いたデジタル模擬患者の事例",
    ...
    # === Slide 13 グループ化4 (角丸四角形 43-50) ===
    "Clinical experience": "臨床体験",
    "CG creation software": "CG制作ソフトウェア",
    "360 degree camera for video production": "映像制作用360度カメラ",
    "VR related equipment": "VR関連機器",
    "High-performance personal computer": "高性能パソコン",
    "Examination of appropriate virtual environment equipment": "適切なバーチャル環境機器の検討",
    "Nursing technical items": "看護技術項目",
    "Examination of alternative nursing practice items": "代替看護実習項目の検討",

    # === Slide 13 グループ化15 > グループ化73 ===
    "VTR (teaching material video)": "VTR（教材ビデオ）",
    "Virtual Reality": "バーチャルリアリティ",
    "Online demonstration": "オンラインデモンストレーション",
    "Textbook": "教科書",
    "(Electronic text)": "（電子テキスト）",
    "On-demand lecture": "オンデマンド講義",
    "Gamification": "ゲーミフィケーション",
    "Realty": "現実感",
    "Reciprocity": "双方向性",
    "Digital online content": "デジタルオンラインコンテンツ",

    # === Slide 13 グループ化101 > グループ化103 (HDカムコーダー) ===
    "HD camcorder": "HDカムコーダー",
    "Shoot in HD quality": "HD画質で撮影",
    "Live broadcast": "ライブ配信",
    "Projector function": "プロジェクター機能",

    # === Slide 13 グループ化101 > グループ化104 (ウェブカメラ) ===
    "Webcam": "ウェブカメラ",
    "Live broadcast by": "ライブ配信が可能",
    " connecting to a PC": " PCに接続して",
    "Small and": "小型で",
    "lightweight": "軽量",

    # === Slide 13 グループ化101 > グループ化105 (360度カメラ1) ===
    "360 degree camera": "360度カメラ",
    "360 degree video": "360度映像",
    " recording": " 撮影",
    "Connect to a PC": "PCに接続して",
    " and perform live": " ライブ配信を実行",

    # === Slide 13 グループ化101 > グループ化106 (360度カメラ2) ===
    "Shoot 360 degrees with": "360度を撮影",
    " 4K image quality": " 4K画質で",
    "VR video production": "VR映像制作",

    # === Slide 13 グループ化101 > グループ化107 (高性能PC) ===
    "High-performance personal computer": "高性能パソコン",
    "4K video editing": "4K映像編集",
    "Control output of ": "複数画面の出力を",
    "multiple screens": "制御",
    "VR operation": "VR操作",

    # === Slide 13 グループ化101 > グループ化108 (HTC VIVE) ===
    "VR actual machine": "VR実機",
    "Operates by": "PCに接続して",
    " connecting to a PC": " 動作",
    "Basically for one": "基本的に",
    " person": " 1人用",
...
def translate_run(run):
    """Translate a single run if its text matches the dictionary."""
    text = run.text
...
def process_shapes(shapes):
    ...
    for shape in shapes:
        ...
        if shape.shape_type == 6:  # GROUP
            try:
                count += process_shapes(shape.shapes)
            except Exception:
                pass
        else:
...
for slide in prs.slides:
    total += process_shapes(slide.shapes)

prs.save(out_path)
print(f"Translated {total} text runs (including group shapes)")

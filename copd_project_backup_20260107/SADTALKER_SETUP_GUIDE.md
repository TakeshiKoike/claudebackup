...
SadTalkerが出力するmpeg4コーデックはブラウザで映像が表示されない場合があります。
**必ずH.264に変換してください：**
...
cd /path/to/videos
for f in *.mp4; do
    ffmpeg -y -i "$f" -c:v libx264 -c:a aac -movflags +faststart "${f%.mp4}_h264.mp4"
...
| 2026/01/07 | シーン2リップシンク動画生成完了。H.264変換手順追加。デモ版HTML動作確認完了。 |
